...
export async function upsertResult(req: Request, res: Response) {
  try {
    ...
    if (admissionNumber && results) return await handleSingleStudent(req, res); // CASE B: Multiple students

    if (Array.isArray(data)) return await handleMultipleStudents(req, res);
  ...
  if (doc) {
    for (let incoming of results) {
      const index = doc.results.findIndex(
        (r) => r.subjectId.toString() === incoming.subjectId.toString()
      );
  ...
  for (let row of data) {
    ...
    for (let item of results) {
      const sub = await Subject.findOneAndUpdate(
        { code: item.subject.toUpperCase() },
        { $setOnInsert: { name: item.subject.toUpperCase() } },
    ...
    let doc = await Result.findOne({ studentObjectId, session, term });
    if (doc) {
      for (let incoming of results) {
        const index = doc.results.findIndex(
          (r) => r.subjectId.toString() === incoming.subjectId.toString()
